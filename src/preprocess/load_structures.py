...
def load_dicom_imageheaders(directory, siUID):
	...
	for i in os.listdir(directory):
		try:
			...
			# safety checks
			correct_patient = (info.StudyInstanceUID == siUID)
			not_RTSTRUCT_or_RTDOSE = (re.search('^RT', info.Modality) is None)
			correct_SOPClassUID = (info.SOPClassUID=="1.2.840.10008.5.1.4.1.1.2")

			if correct_patient and not_RTSTRUCT_or_RTDOSE and correct_SOPClassUID:
